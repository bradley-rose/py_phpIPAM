...
class IPAM:
    ...
    def patch(self, url, payload):
        ...
        result = requests.patch(url, json=payload, verify=False, headers={'token':self.token}).json()
        return result
    ...
    def delete(self,url):
        ...
        result: int
            HTTP status code
        """
        result = requests.delete(url, verify=False, headers={'token':self.token}).json()
        return result
    ...
    def getSubnetAddresses(self, subnet):
        """
        Description
        -----------
    ...
    def updateLocation(self, payload):
        ...
        result: int
            HTTP status code of operation.
        """
        url = self.urlBase + "tools/locations/" + str(payload['id'])
        del payload['id']
        results = self.patch(url, payload)
        return results
    ...
    def getDeviceTypes(self, specific=False):
        """
        Description
        -----------
    ...
    def getDevicesByType(self, typeId):
        """
        Description
        -----------
        Obtains device type objects from phpIPAM.
        ...
        if specific=False:
            ...
            results: dictionary
                Obtains one specific device type object and its key:value attributes.
        """
        url = self.urlBase + 'tools/device_types/' + str(typeId) + "/devices/"
        results = self.get(url)
        return results

    def getSections(self, specific=False):
        """
        Description
        -----------
    ...
    def getAddressFirstFree(self,subnetId):
        """
        Description
        -----------
        Obtains the first free address from a defined subnet.
        ...
        result: dict {
            ...
            success: bool
                Job success boolean
            data: ip
                First free IP address in target subnet.
            time: string
                Resulting time it took from query to response for IPAM.
        }
        """
        url = self.urlBase + "/addresses/first_free/" + str(subnetId)
        results = self.get(url)
        return results
    ...
    def updateAddress(self, payload):
        """
        Description
        -----------
    ...
    def getAddressBySubnet(self, subnetId):
        """
        Description
        -----------
        Obtains all IP addresses belonging to a subnet.

        Parameters
        ----------
        subnetId: int
            An integer value representing the subnet object.
        ...
        result: dict {
            ...
            success: bool
                Job success boolean
            data: list [
                List of IP address objects in subnet
            ]
            time: string
                Time from request to response for IPAM to process.
        }
        """
        url = self.urlBase + 'subnets/' + str(subnetId) + "/addresses"
        results = self.get(url)
        return results
    ...
    def deleteAddress(self, addressId):
        ...
        Description
        -----------
        Removes address object.

        Parameters
        ----------
        addressId: int
            An integer representing the phpIPAM id for the target adress.
        ...
        result: int
            if result == 200:
                ...
                HTTP error code, investigate as appropriate.
        """
        url = self.urlBase + 'addresses/' + str(addressId)
        results = self.delete(url)
        return results

    def getChildSubnets(self, id):
        """
        Description
        -----------
        Obtains child subnets of a given subnet.

        Parameters
        ----------
        id: int
            Integer value representing parent subnet.
        ...
        result: dict {
            ...
            success: bool
                Job success boolean
            data: dict{
                subnet objects for each child subnet.
            }
            time: string
                Resulting time it took from query to response for IPAM.
        }
        """
        url = self.urlBase + 'subnets/' + str(id) + "/slaves" 
        results = self.get(url)
        return results